...
public:
    ...
    void Invoke(const Event &event) {
        ...
        if (event.type == MouseEvent::MOUSE_DOWN) {
            ...
            switch (st->keyCode) {
                case Qt::Key_1: {
                    Vector2D cam = Camera::gi()->getScaling();
                    Camera::gi()->setScaling(Vector2D(cam.x - 5, cam.y - 5));
                    break;
                }
                case Qt::Key_2: {
                    Vector2D cam = Camera::gi()->getScaling();
...
public:
    ...
    ArrowsView(std::vector<WorkView *> *list) {
        ...
        arrow->setRotationZ(M_PI);
        this->list = list;
    }

    void render(QPainter *render2d, const Matrix &base, bool new_tick, double interpolation) {
        ...
        for (int i = 0; i < list->size(); ++i) {
            ...
            bool hit = view_pos.x >= 0 && view_pos.x <= Camera::gi()->screen_width && view_pos.y >= 0 &&
                       view_pos.y <= Camera::gi()->screen_height;
            if (!hit) {
...
public:
    ...
    void render(QPainter *render2d, const Matrix &base, bool new_tick, double interpolation) {
        ...
        for (std::map<int, Player *>::iterator i = map->begin(); i != map->end(); ++i) {
            ...
            render2d->setPen(pen);
            QString text = QString("Player #") + QVariant(player->id_player).toString() + ": " +
                           QVariant(player->count_kills).toString() + "-" + QVariant(player->count_dieds).toString();
...
public:
    ...
    void render(QPainter *render2d, const Matrix &base, bool new_tick, double interpolation) {
        ...
        if (i != map->end()) {
            ...
            render2d->drawText(camera->screen_width - 150, camera->screen_height - 50, 100, 30, Qt::AlignRight,
                               QVariant(player->health).toString() + "%");
            ...
            render2d->drawText(70, camera->screen_height - 65, 70, 20, Qt::AlignCenter,
                               QVariant(floor(player->speed * 3.6)).toString());
        }
    }
