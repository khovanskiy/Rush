...
struct CrossingResult2D
{
    ...
    CrossingResult2D(bool crossing, Point2D const & center, Vector2D normal)
        : center(center), crossing(crossing), normal(normal)
    {
    }
};
...
public:
    ...
    virtual double getDepth(Point2D const & point) const = 0;
...
struct Segment2D : public Shape2D
{
    ...
    virtual double getDepth(const Point2D &point) const;
